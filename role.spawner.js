...
var roleSpawner = {
    spawnBiggestPossible: function(spawn, creep_type) {
        ...
        if(possible_blueprints.length > 0) {
            var result = spawn.spawnCreep(possible_blueprints[possible_blueprints.length - 1].parts, creep_type + " " + Game.time.toString(), {memory: {role: creep_type}});
            console.log(creep_type + " spawn result: " + result);
            return result;
        }
